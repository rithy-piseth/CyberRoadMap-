    ...
    <div style={{ minHeight: '100vh', background: t.bg, color: t.text, fontFamily: "'Rajdhani',sans-serif", overflowX: 'hidden', position: 'relative', transition: 'all 0.3s' }}>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .info-row{display:flex;justify-content:space-between;align-items:center;padding:1rem 0;border-bottom:1px solid ${t.tealBorder};gap:1rem;flex-wrap:wrap}
        .info-row:last-child{border-bottom:none}
        .info-value{color:${t.text};font-weight:600;word-break:break-all}
        ...
        @media(max-width: 600px) {
          .profile-main { padding: 5rem 1.2rem 3rem !important; }
          .rec-card-inner { flex-direction: column !important; align-items: flex-start !important; }
          .rec-card-btn { width: 100% !important; text-align: center; margin-top: 0.5rem; }
      ...
      {loading ? (
        ...
        <main className="profile-main" style={{ position: 'relative', zIndex: 1, padding: '6rem 2rem 4rem', maxWidth: 800, margin: '0 auto' }}>
          ...
          <div className={loaded ? 'fade-0' : ''} style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
            ...
            <h1 style={{ fontFamily: "'Orbitron',monospace", fontSize: 'clamp(1.3rem, 4vw, 2rem)', fontWeight: 900, color: t.text, marginBottom: '0.3rem' }}>{profile.username}</h1>
            <p style={{ color: t.textDim, fontSize: '0.9rem', wordBreak: 'break-all' }}>{profile.email}</p>
          </div>
              ...
              <div className="rec-card-inner" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontFamily: "'Orbitron',monospace", fontSize: 'clamp(0.85rem, 2vw, 1.1rem)', fontWeight: 700, color: teamColor }}>{profile.recommended_path}</div>
                ...
                <button
                  ...
                  style={{ background: teamColor, border: 'none', color: '#fff', fontFamily: "'Rajdhani',sans-serif", fontSize: '0.9rem', fontWeight: 600, cursor: 'pointer', padding: '0.6rem 1.5rem', borderRadius: 8, letterSpacing: '1px', transition: 'all 0.2s', whiteSpace: 'nowrap' }}
          ...
          <div className={loaded ? 'fade-2' : ''} style={{ background: t.card, border: `1px solid ${t.tealBorder}`, borderRadius: 16, padding: '1.5rem', marginBottom: '1.5rem', backdropFilter: 'blur(10px)' }}>
            ...
            <div className="info-row"><span style={{ color: t.textDim, fontSize: '0.9rem', flexShrink: 0 }}>Username</span><span className="info-value">{profile.username}</span></div>
            <div className="info-row"><span style={{ color: t.textDim, fontSize: '0.9rem', flexShrink: 0 }}>Email</span><span className="info-value">{profile.email}</span></div>
            <div className="info-row"><span style={{ color: t.textDim, fontSize: '0.9rem', flexShrink: 0 }}>Member since</span><span className="info-value">{formatDate(profile.created_at)}</span></div>
            <div className="info-row"><span style={{ color: t.textDim, fontSize: '0.9rem', flexShrink: 0 }}>Career path</span><span style={{ color: profile.recommended_path ? t.teal : t.textDim, fontWeight: 600 }}>{profile.recommended_path || 'Not set yet'}</span></div>
          </div>

          {/* Action buttons */}
          <div className={`action-btns ${loaded ? 'fade-3' : ''}`} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
