      ...
      {loading ? (
        ...
        <main style={{ position:'relative', zIndex:1, padding:'6rem 2rem 4rem', maxWidth:800, margin:'0 auto' }}>
          ...
          <div className={loaded?'fade-0':''} style={{ textAlign:'center', marginBottom:'2.5rem' }}>
            ...
            <h1 style={{ fontFamily:"'Orbitron',monospace", fontSize:'clamp(1.5rem,4vw,2rem)', fontWeight:900, color:t.text, marginBottom:'0.3rem' }}>
              {profile.username}
            </h1>
          ...
          {profile.recommended_path && (
            <div className={loaded?'fade-1':''} style={{ background:teamDim, border:`1px solid ${teamBorder}`, borderRadius:16, padding:'1.5rem', marginBottom:'1.5rem', position:'relative', overflow:'hidden' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'1rem' }}>
                ...
                <button
                  onClick={() => navigate(`/specialist/${profile.recommended_path.toLowerCase().replace(/\s+/g,'-')}`)}
          ...
          <div className={loaded?'fade-2':''} style={{ background:t.card, border:`1px solid ${t.tealBorder}`, borderRadius:16, padding:'1.5rem', marginBottom:'1.5rem', backdropFilter:'blur(10px)' }}>
            ...
            <div className="info-row">
              <span style={{ color:t.textDim, fontSize:'0.9rem' }}>Username</span>
              <span style={{ color:t.text, fontWeight:600 }}>{profile.username}</span>
            </div>
            <div className="info-row">
              <span style={{ color:t.textDim, fontSize:'0.9rem' }}>Email</span>
              <span style={{ color:t.text, fontWeight:600 }}>{profile.email}</span>
            </div>
            <div className="info-row">
              <span style={{ color:t.textDim, fontSize:'0.9rem' }}>Member since</span>
              <span style={{ color:t.text, fontWeight:600 }}>{formatDate(profile.created_at)}</span>
            </div>
            <div className="info-row">
              <span style={{ color:t.textDim, fontSize:'0.9rem' }}>Career path</span>
              <span style={{ color:profile.recommended_path?t.teal:t.textDim, fontWeight:600 }}>
                {profile.recommended_path || 'Not set yet'}
              </span>
            </div>
          </div>
          ...
          <div className={loaded?'fade-3':''} style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem' }}>
            ...
            <button
              ...
              style={{ gridColumn:'1/-1', background:t.errorBg, border:`1px solid ${t.errorBorder}`, color:t.errorText }}
