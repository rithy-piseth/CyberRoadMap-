...
export default function Login() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: t.bg, transition: 'background 0.3s', fontFamily: "'Rajdhani',sans-serif" }}>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        @media(max-width: 480px) {
          .auth-card { padding: 1.8rem 1.3rem !important; }
          .auth-wrapper { padding: 5rem 1rem 3rem !important; }
        }
      `}</style>
      ...
      <div className="auth-wrapper" style={{ position: 'relative', zIndex: 1, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '6rem 1.5rem 4rem' }}>
        <div style={{ width: '100%', maxWidth: 420, animation: 'fadeSlideUp 0.6s ease both' }}>
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            <div style={{ fontFamily: "'Orbitron',monospace", fontSize: 'clamp(1.2rem, 3vw, 1.5rem)', fontWeight: 900, color: t.teal, marginBottom: '0.5rem' }}>CyberRoadMap</div>
            <div style={{ color: t.textDim, fontSize: '0.9rem' }}>Sign in to your account</div>
          </div>

          <div className="auth-card" style={{ background: t.card, border: `1px solid ${t.tealBorder}`, borderRadius: 16, padding: '2.5rem', backdropFilter: 'blur(12px)', boxShadow: darkMode ? '0 20px 60px rgba(0,0,0,0.3)' : '0 10px 40px rgba(0,0,0,0.08)', transition: 'all 0.3s' }}>
