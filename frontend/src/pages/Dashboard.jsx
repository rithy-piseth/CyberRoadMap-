...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Load profile separately so it won't block teams
    API.get('/api/profile')
  ...
  return (
    <div style={{ minHeight: '100vh', background: t.bg, color: t.text, fontFamily: "'Rajdhani', sans-serif", overflowX: 'hidden', position: 'relative', transition: 'all 0.3s' }}>
      ...
      <main style={{ position:'relative', zIndex:1, padding:'6rem 2rem 4rem', maxWidth:1200, margin:'0 auto' }}>
        ...
        {/* Teams grid */}
        {loading ? (
          <div style={{ textAlign:'center', color:t.teal, fontFamily:"'Orbitron',monospace", fontSize:'0.9rem', letterSpacing:'2px', padding:'4rem' }}>
            Loading...
          </div>
        ) : (
              ...
              return (
                <div key={team.name} className={`team-card ${loaded?`fade-${ti+2}`:''}`} style={{ background:t.card, border:`1px solid ${accentBorder}`, padding:'2rem' }}>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:'1rem', marginBottom:'1.5rem' }}>
                    <div style={{ width:52, height:52, borderRadius:12, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.6rem', background:accentDim, border:`1px solid ${accentBorder}` }}>
                      {blue?'🛡️':'⚔️'}
                    </div>
                    <div>
        ...
        <div style={{ textAlign:'center', marginTop:'3rem', padding:'2rem', border:`1px solid ${t.tealBorder}`, borderRadius:16, background:t.card, backdropFilter:'blur(10px)' }}>
          ...
          <button
            onClick={() => navigate('/assessment')}
            style={{ background:t.teal, border:'none', color:t.bg, fontFamily:"'Orbitron',monospace", fontSize:'0.85rem', fontWeight:700, cursor:'pointer', padding:'0.8rem 2.5rem', borderRadius:8, letterSpacing:'2px', transition:'all 0.2s' }}
