...
export default function Dashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: t.bg, color: t.text, fontFamily: "'Rajdhani',sans-serif", overflowX: 'hidden', position: 'relative', transition: 'all 0.3s' }}>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        @media(max-width: 768px) {
          ...
          .dashboard-main { padding: 5rem 1.2rem 3rem !important; }
        }
        @media(max-width: 600px) {
          .rec-card-inner { flex-direction: column !important; align-items: flex-start !important; }
          .rec-card-btn { width: 100% !important; text-align: center; }
          .specialist-card { padding: 0.8rem 1rem; }
          .specialist-card:hover { transform: none; }
          .team-card { padding: 1.5rem !important; }
          .not-sure-box { padding: 1.5rem !important; }
        }
      `}</style>
      ...
      <main className="dashboard-main" style={{ position: 'relative', zIndex: 1, padding: '6rem 2rem 4rem', maxWidth: 1200, margin: '0 auto' }}>

        {/* Header */}
        <div className={loaded ? 'fade-0' : ''} style={{ marginBottom: '2rem' }}>
          ...
          <h1 style={{ fontFamily: "'Orbitron',monospace", fontSize: 'clamp(1.4rem, 4vw, 2.8rem)', fontWeight: 900, color: t.text, marginBottom: '0.5rem' }}>
            Welcome back, <span style={{ color: t.teal }}>{username}</span>
          </h1>
          <p style={{ color: t.textDim, fontSize: 'clamp(0.88rem, 1.5vw, 1rem)' }}>Explore cybersecurity career paths and find your learning roadmap.</p>
        </div>

        {/* Recommended path */}
        {profile?.recommended_path && (
          <div className={loaded ? 'fade-1' : ''} style={{ marginBottom: '2rem' }}>
            <div className="rec-card" style={{ background: profile.team?.toLowerCase().includes('red') ? t.redDim : t.blueDim, border: `1px solid ${profile.team?.toLowerCase().includes('red') ? t.redBorder : t.blueBorder}` }}>
              <div className="rec-card-inner" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontFamily: "'Orbitron',monospace", fontSize: 'clamp(0.9rem, 2vw, 1.1rem)', fontWeight: 700, color: t.teal }}>{profile.recommended_path}</div>
              ...
              return (
                <div key={team.name} className={`team-card ${loaded ? `fade-${ti + 2}` : ''}`} style={{ background: t.card, border: `1px solid ${accentBorder}`, padding: '2rem' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
                    <div style={{ width: 52, height: 52, borderRadius: 12, overflow: 'hidden', border: `1px solid ${accentBorder}`, flexShrink: 0 }}>
                  ...
                  {team.specialists.map((s, i) => (
                    <div key={i} className="specialist-card" onClick={() => navigate(`/specialist/${s.slug}`)}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 'clamp(0.85rem, 1.5vw, 0.95rem)', fontWeight: 600, color: t.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.name}</div>
                        {s.income && <div style={{ fontSize: '0.78rem', color: t.textDim, marginTop: '0.1rem' }}>{s.income}</div>}
                      </div>
                      <div style={{ color: accent, fontSize: '1rem', opacity: 0.6, flexShrink: 0 }}>→</div>
                    </div>
                  ))}
                  <button
        ...
        {/* Not sure box */}
        <div className="not-sure-box" style={{ textAlign: 'center', marginTop: '2.5rem', padding: '2rem', border: `1px solid ${t.tealBorder}`, borderRadius: 16, background: t.card, backdropFilter: 'blur(10px)' }}>
          <div style={{ fontFamily: "'Orbitron',monospace", fontSize: 'clamp(0.88rem, 2vw, 1rem)', fontWeight: 700, color: t.teal, marginBottom: '0.5rem' }}>Not sure which path?</div>
          <p style={{ color: t.textDim, fontSize: 'clamp(0.82rem, 1.5vw, 0.9rem)', marginBottom: '1.2rem' }}>Let our AI analyze your skills and interests to recommend the perfect career path.</p>
          <button
            onClick={() => navigate('/assessment')}
